...
class Frame_Gerenate(Gerenate):
    ...
    def RenderBitmap(self, unicode, font_size, ud, lr):
        ...
        base_offset = (RENDERSIZE - font_size) // 2
        ox = base_offset + lr
        oy = base_offset + ud
        ...
        for y in range(base_offset, base_offset + font_size):
            ...
            for x in range(base_offset, base_offset + font_size):
                ...
                gray = (color.red*38 + color.green*75 + color.blue*15) >> 7
                if gray > 192:
                    line += '00'
                elif gray > 128:
                    line += '01'
    ...
    def GetBitmapAttribute(self, bitmap, unicode, font_spacing, font_size):
        ...
        for y in range(y_start, y_end + 1):
            valid_bitmap.append(bitmap[y][x_start * 2 : x_end * 2 + 2])
...
class Frame_NewFont(NewFont):
    ...
    def Render_Preview(self):
        ...
        bmp = wx.Bitmap(RENDERSIZE, RENDERSIZE)
        dc = wx.MemoryDC()
        dc.SelectObject(bmp)
        dc.Clear()

        dc.SetFont(font)
        base_offset = (RENDERSIZE - self.fontsize) // 2
        x = base_offset + LR
        y = base_offset + UD
        dc.DrawText(chr(unicode), x, y)

        xy = base_offset // 2
        wh = RENDERSIZE - base_offset
